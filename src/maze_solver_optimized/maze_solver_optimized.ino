// This is the optimized iteration of my maze solver robot developed in the Arduino IDE.
// It improves upon the standard version by tuning movement parameters and navigation to achieve faster maze completion times while maintaining autonomous operation.
// This version prioritizes speed and performance over total reliability.
...
const unsigned long STUCK_TIMEOUT = 900;   // This is how long the sensor readings can remain unchanged until the code checks if the robot is stuck (1.5 seconds / 1500 ms)
const int STUCK_TOLERANCE = 30;             // This is our tolerance for how much of a difference the sensor readings can show while checkIfStuck() is true

int lastFrontDist = -1, lastRightDist = -1, lastLeftDist = -1;
unsigned long lastChangeTime = 0;

bool checkIfStuck() {
  // If the difference between last sensor value and present sensor value changed more than tolerance (15), reset the timer and set checkIfStuck False
...
void setup() {
  ...
  WDT.begin(3000); // Start Watchdog Timer for 3 seconds
